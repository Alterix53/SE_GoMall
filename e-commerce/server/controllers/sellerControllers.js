...
export const applyForSeller = async (req, res) => {
    try {
        ...
        if (existing) {
            ...
            return res.status(400).json({ success: false, message, existingStatus: existing.status });
        }
        ...
        if (error.name === 'ValidationError') {
            ...
            return res.status(400).json({ success: false, message: 'Dữ liệu không hợp lệ: ' + validationErrors.join(', ') });
        }
        if (error.code === 11000) {
            return res.status(400).json({ success: false, message: 'Hồ sơ đã tồn tại cho user này' });
        }
...
export const updateSeller = async (req, res) => {
    try {
        ...
        const seller = await Seller.findByIdAndUpdate(id, updateData, { new: true }).populate('userID');
        if (!seller) {
            return res.status(404).json({ success: false, message: 'Seller not found' });
        }
        res.json({ success: true, message: 'Seller updated successfully', data: seller });
    } catch (error) {
...
export const deleteSeller = async (req, res) => {
    try {
        const { id } = req.params;
        const seller = await Seller.findByIdAndUpdate(id, { isActive: false }, { new: true }).populate('userID');
        if (!seller) {
            return res.status(404).json({ success: false, message: 'Seller not found' });
        }
        await User.findByIdAndUpdate(seller.userID._id, { $pull: { role: 'seller' } });
        res.json({ success: true, message: 'Seller deactivated successfully', data: seller });
    } catch (error) {
...
export const getSellerByUserId = async (req, res) => {
    try {
        ...
        const seller = await Seller.findOne({ userID: userId })
            .populate('userID', 'username email fullName phoneNumber address');
        if (!seller) {
            return res.status(404).json({ success: false, message: 'Seller not found for this user' });
        }
        res.json({ success: true, data: seller });
    } catch (error) {
...
export const checkCurrentUserSellerStatus = async (req, res) => {
    try {
        ...
        switch (seller.status) {
            case 'pending':
                message = `Hồ sơ của bạn đã được nộp vào ${new Date(seller.createdAt).toLocaleDateString('vi-VN')} và đang chờ duyệt.`;
                break;
            case 'approved':
                message = `Hồ sơ của bạn đã được duyệt vào ${new Date(seller.updatedAt).toLocaleDateString('vi-VN')}.`;
                break;
            case 'rejected':
                message = `Hồ sơ của bạn đã bị từ chối vào ${new Date(seller.updatedAt).toLocaleDateString('vi-VN')}.`;
                break;
            case 'suspended':
                message = `Tài khoản seller của bạn đã bị tạm ngưng vào ${new Date(seller.updatedAt).toLocaleDateString('vi-VN')}.`;
                break;
            default:
