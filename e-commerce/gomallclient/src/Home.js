...
function Home() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      try {
        const flashRes = await productAPI.getFlashSaleProducts({ limit: 8 });
        const flash = flashRes?.products || [];
        const mappedFlash = flash.map((p) => ({
          _id: p._id,
          name: p.name,
          price: p?.price?.sale ?? p?.price?.original ?? 0,
          originalPrice: p?.price?.original ?? 0,
          discount:
            p?.price?.original && (p?.price?.sale ?? 0)
      ...
      try {
        const topRes = await productAPI.getTopProducts('bestseller', { limit: 12 });
        const top = topRes?.products || [];
        const mappedTop = top.map((p) => ({
          _id: p._id,
          name: p.name,
          price: p?.price?.sale ?? p?.price?.original ?? 0,
          originalPrice: p?.price?.original ?? 0,
      ...
      try {
        const allRes = await productAPI.getProducts({ limit: 100 });
